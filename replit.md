...
- **Credit Control System**: Multi-tenant monthly credit system. Super Admin sets `credit_per_user` per tenant (default 500). AI email generation costs 2 credits; prospect enrichment costs 1 credit. Blocking enforced server-side with 402 response. Monthly auto-reset with no rollover. Real-time credit balance widget on the SDR dashboard. Credit audit log with full deduction history. Tables: `user_credits`, `credit_logs`. Credit limit editable by Super Admin in tenant detail page and on tenant creation.
