...
- **Guaranteed Delivery Pipeline**: Auto-retry system with exponential backoff (PENDING > 10 min → RETRY, RETRY > 3 → FAIL + notify). Dead-letter queue API at `/api/admin/email-queue/dead-letter` exposes failed emails with reasons. Manual retry trigger at `/api/admin/email-queue/retry-stuck`. Every email action logged with reason/source for auditability.
