...
import { Loader2, Search, User, Building2, MapPin } from "lucide-react";
...
export function ProspectSelector({ selectedIds = [], onSelectionChange }: ProspectSelectorProps) {
  ...
  const selectAll = () => {
    onSelectionChange(filteredProspects.map((p: any) => p.id));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-muted-foreground">
        {safeSelectedIds.length} of {filteredProspects.length} prospects selected
      </div>
          ...
          {filteredProspects.map((prospect: any) => (
            <Card
              ...
              className={`p-4 cursor-pointer transition-colors hover:bg-muted/50 ${
                safeSelectedIds.includes(prospect.id) ? 'border-primary bg-primary/5' : ''
              }`}
            ...
            >
              <div className="flex items-start gap-3">
                <div onClick={(e) => e.stopPropagation()}>
                  <Checkbox
                    checked={safeSelectedIds.includes(prospect.id)}
                    onCheckedChange={(checked) => {
                      toggleProspect(prospect.id);
                    }}
